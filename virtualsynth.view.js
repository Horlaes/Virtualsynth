function AudioSynthView(panel) {

    var isMobile = !!navigator.userAgent.match(/Android|BlackBerry|iPhone|iPad|iPod|Opera Mini|IEMobile/i),
        evtListener;
    if (isMobile) {
        evtListener = ['touchstart', 'touchend'];
    } else {
        evtListener = ['mousedown', 'mouseup'];
    }
    ...
    var fnChangeOctave = function(x) {
        ...
        var octaveName = panel.content.querySelector('.OCTAVE_LABEL');
        ...
        panel.content.querySelector('.OCTAVE_LOWER').innerHTML = __octave - 1;
        panel.content.querySelector('.OCTAVE_UPPER').innerHTML = __octave + 1;
        panel.content.querySelector('.notesPlayed').innerHTML = '';
        panel.content.querySelector('.notesPlayedhidden').innerHTML = '';
        panel.content.querySelector('.playback').disabled = true;
        panel.content.querySelector('.clear').disabled = true;
    ...
    var fnCreateKeyboard = function(keyboardElement) {
        ...
        visualKeyboard = panel.content.querySelector('.keyboard-holder');
        selectSound = panel.content.querySelector('.sound');
            ...
            for (var n in notes) {
                if (n[2] != 'b') {
                    ...
                    label.innerHTML = '<b>' + String.fromCharCode(reverseLookupText[n + ',' + i]) + '</b>' + '<br /><br />' + n.substr(0, 1) +
                        '<span class="OCTAVE_LABEL" value="' + i + '">' + (__octave + parseInt(i)) + '</span>' + (n.substr(1, 1) ? n.substr(1, 1) : '');
    ...
    var fnPlayNote = function(note, octave) {
        var src = __audioSynth.generate(selectSound.value, note, octave, 2);
    ...
    var fnPlayKeyboard = function(e) {
        ...
        if (keyboard[e.keyCode]) {
            ...
            panel.content.querySelector('.playback').disabled = false;
            panel.content.querySelector('.clear').disabled = false;
            if (panel.content.querySelector('.notesPlayed').innerHTML != '') {
                panel.content.querySelector('.notesPlayed').innerHTML += ' ';
            }
            if (panel.content.querySelector('.notesPlayedhidden').innerHTML != '') {
                panel.content.querySelector('.notesPlayedhidden').innerHTML += ' ';
            }
            panel.content.querySelector('.notesPlayed').innerHTML += note + (__octave + octaveModifier);
            panel.content.querySelector('.notesPlayedhidden').innerHTML += keyboard[e.keyCode];
        } else {
            return false;
        }
    };
    ...
    var playback = function p() {
        ...
        panel.content.querySelector('.clear').disabled = true;
        var i = 1;
        var noteslist = panel.content.querySelector('.notesPlayed').innerHTML;
        var list = panel.content.querySelector('.notesPlayedhidden').innerHTML.split(' ');
        var listvisible = panel.content.querySelector('.notesPlayed').innerHTML.split(' ');
        ...
        (function myLoop(i) {
            setTimeout(function() {
                if (index < list.length) {
                    ...
                    panel.content.querySelector('.notesPlayed').innerHTML = noteslist.replace(listvisible[index], '<font color="red">' + listvisible[index] + '</font>');
                    if (index > 0) {
                    ...
                    panel.content.querySelector('.playback').disabled = false;
                    panel.content.querySelector('.clear').disabled = false;
                    panel.content.querySelector('.notesPlayed').innerHTML = noteslist;
                }
            }, 1000);
        })(list.length + 1);
    ...
    panel.content.querySelector('.minusOCTAVE').addEventListener('click', function() {
        fnChangeOctave(-1);
    });
    panel.content.querySelector('.plusOCTAVE').addEventListener('click', function() {
        fnChangeOctave(1);
    });
    panel.content.querySelector('.playback').addEventListener('click', function() {
        playback();
    });
    panel.content.querySelector('.clear').addEventListener('click', function() {
        clearplayback();
    });
